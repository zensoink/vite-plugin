...
import { PACKAGE_JSON_PATH, ZENSO_CONFIG_PATH } from './constants.js';
 ...
 * `version`, or `zenso.config.json` lacks a valid `owner/repo` `id` — all are
 * merged here and required by the backend validator, so the build fails fast
 * instead of producing a ZIP the upload rejects.
...
export function buildManifestJson(): Record<string, any> {
    ...
    // version SemVer, id owner/repo); full-manifest validation stays the backend's job
    ...
    if (typeof zensoConfig.id !== 'string' || !/^[a-z0-9]+\/[a-z0-9-]+$/.test(zensoConfig.id)) {
        throw new Error('[zenso]: zenso.config.json must provide an "id" like "zenso/my-plugin" (merged into dist/manifest.json, required by the backend)');
    }
    return {
